...
export async function fetchCommunityStats(puzzleId: string): Promise<CommunityComparison | null> {
  try {
    ...
    type CommunityRow = { outcome: string; rank: string | null; total_points: number | null; turns_used: number | null; characters_used: number | null; character_ids: string[] | null };
    ...
    const avgScore = victories.length > 0
      ? victories.reduce((sum: number, r) => sum + (r.total_points || 0), 0) / victories.length
      : 0;

    // Average turns (all attempts)
    const avgTurns = rows.length > 0
      ? rows.reduce((sum: number, r) => sum + (r.turns_used || 0), 0) / rows.length
      : 0;
...
export async function fetchPuzzleStats(puzzleId: string): Promise<PuzzleStats | null> {
  try {
    ...
    type PuzzleStatsRow = { outcome: string; rank: string | null; total_points: number | null; turns_used: number; characters_used: number; character_ids: string[] | null; defeat_reason: string | null; defeat_turn: number | null };
    const rows = data as PuzzleStatsRow[];
    ...
    return {
      ...
      avgScore: victories.length > 0
        ? Math.round(victories.reduce((s: number, r) => s + (r.total_points || 0), 0) / victories.length)
        : 0,
      avgTurns: Math.round(rows.reduce((s: number, r) => s + r.turns_used, 0) / rows.length * 10) / 10,
      avgCharactersUsed: Math.round(rows.reduce((s: number, r) => s + r.characters_used, 0) / rows.length * 10) / 10,
      rankDistribution: rankDist,
...
export async function fetchOverviewStats(
  ...
  try {
    ...
    type OverviewRow = { puzzle_id: string; puzzle_date: string | null; outcome: string; player_id: string };
