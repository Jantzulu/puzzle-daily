...
export function executeTurn(gameState: GameState): GameState {
  ...
  // Headless: instant resolution for solver/validator
  // Non-headless: per-frame collision in updateProjectiles (called from AnimatedGameBoard
  // and once before executeTurn in Game.tsx to ensure determinism)
  ...
  for (const proj of gameState.activeProjectiles) {
    ...
    if (effectivelyCharacterFired && !entityHitAndStopped) {
      if (isHealingProjectile) {
        ...
        // Only check current positions - entities are hit where they actually are
