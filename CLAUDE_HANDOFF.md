...
Last Updated: July 14, 2026 (TRIGGER OVERHAUL shipped in three slices: team-relative events + read-time mapping, REPEAT_UNTIL segment blocks, rich condition vocabulary — see "Recently completed". The ally triggered-action bug is FIXED. NEXT SESSION IS PRE-BRIEFED: hit-stamp conditions — see "Next session — start here". NOTE: the July 1–12 work — engine audit sweeps 1–10, summon/necromancy/vessels, Phase E homing helpers, strafe actions, contact redesign — is chronicled in the user-memory `in-progress.md`, not here; this doc's session log resumes at June 30 below.)
...
**HIT-STAMP TRIGGER CONDITIONS (user design 2026-07-14, the last piece of the trigger overhaul).** "Hit by spell / melee / projectile" and the mirror "landed a spell/melee/projectile hit", each with THREE freshness windows the user chose: **previous action** (stamp within the last turn — reactive), **this cycle** (happened since the current behavior-loop iteration started; refreshes every cycle so it can fire repeatedly), **ever** (once true, always true). Session brief:

1. **Mechanism (agreed with the user):** when damage lands, stamp the victim (`hitStamps`) and the attacker (`dealtStamps`) with the turn number per delivery kind (melee / projectile / contact / any). Conditions are then pure predicates again ("was I hit by a projectile on this turn or the previous one?"). Stamps are plain JSON fields on PlacedCharacter/PlacedEnemy → replay snapshots carry them; all damage flows through the shared engine paths → solver parity comes free. Contact (Thorns/Trample) gets its own kind so authors can distinguish.
2. **"This cycle" needs `cycleStartTurn`:** stamped whenever the entity's action pointer wraps to its segment start (both REPEAT and REPEAT_UNTIL loop-backs — see `planRepeatUntil` in simulation.ts). Condition = stamp >= cycleStartTurn. Note `repeatUntilCounts` (shipped 2026-07-14) already does per-block bookkeeping — same pattern, new-object writes (never in-place mutation; replay snapshots share references).
3. **The minefield — this touches the SACRED damage path.** `applyDamageToEntity` is the centralized damage function; stamping sites must cover every delivery (melee, cone, AOE, projectile real+headless via `applyEntityHit`, contact, tile, DOT, push, deflect) — the exact axis audit sweep 1 chased. Victim stamps are easy (the target is the real state object). **Attacker stamps are the hard part:** the acting entity is a COPY (newCharacter / tempChar wrapper), so either stamp the original via lookup (beware duplicate same-asset ids — instanceKey territory) or ride the wrapper copy-backs (field-by-field literals in THREE places in simulation.ts + the trigger phase; every new field must be added manually, both directions). Do it slowly, one delivery kind per commit, pin each.
4. **Window semantics to honor:** "previous action" = stamp >= currentTurn - 1 (projectiles resolve AFTER actions, so a turn-N projectile hit is reacted to on turn N+1); "ever" = stamp exists. Editor: the three windows are a picker on the hit conditions only (option metadata pattern in BehaviorSequenceBuilder's TRIGGER_EVENT_OPTIONS is ready for it).
5. **Test seams** (same as ever, pinned in the test files): authored trigger actions are executionMode 'parallel'; `testMode: true` skips win/lose; park a WAIT+REPEAT hero; run vitest from puzzle-game/. The summon-ping observable (trigger-events.test.ts) counts firings without needing a damage target; note summoned spawnlings join the CASTER's party and block their spawn tile for later pings — use movement observables when counting repeats.

**Decided 2026-07-14 (do not relitigate):** BASE-party sensing (charm-blind); same-team events exclude self but SEE stealthed teammates; legacy events map at read time by authoring side (no migration); REPEAT_UNTIL uses SEGMENT semantics (each block loops back only to the previous block); `repeated_times` means "the segment has run N times" and resets on fall-through; counts are a census (stealth doesn't hide). Also standing from 2026-07-13: allies collecting items + scoring is intended.
...
- **Next:** hit-stamp conditions (see "Next session — start here").
