...
**PERF SWEEP SHIPPED (`26c2ef1`) — the next user step: load the play page with `?perfsweep=1`** (auto-runs after ~1s of board animation; ~35s total; best scrolled down to the janky view), then screenshot the HUD results table. 7 configs × 5s: `base` / `-board` (freeze) / `-sat` (pause torch+thumbnails) / `-smil` (pause SVG animation — the banner test) / `-css` (pause CSS animations) / `-svg` (hide all SVGs) / `bare` (everything off). All overrides volatile (aborted sweep leaves nothing). Read: `-smil` recovering fps → rework BannerMesh's mobile mode (static hem or slow the ripple to steps, split the drop-shadow onto a filterless polygon); `-css` recovering → hunt index.css's continuous animations; `-svg` beyond `-smil` → static SVG composite cost; only `bare` recovering → canvas-acceleration pressure (reduce canvas count). Other queued suspects: static-bake sig includes `currentTurn` (full rebake+upload every turn — hitch suspect); `updateProjectiles` still samples Date.now() internally (thread the loop's `now` if projectile motion judders).
