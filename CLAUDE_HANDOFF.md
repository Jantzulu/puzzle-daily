...
Last Updated: July 14, 2026, second session (HIT-STAMP CONDITIONS shipped in four slices — the last piece of the trigger overhaul; see "Recently completed". The whole July-14 trigger batch — team-relative events, REPEAT_UNTIL, rich conditions, hit stamps — now AWAITS USER TESTING. NOTE: the July 1–12 work — engine audit sweeps 1–10, summon/necromancy/vessels, Phase E homing helpers, strafe actions, contact redesign — is chronicled in the user-memory `in-progress.md`, not here; this doc's session log resumes at June 30 below.)
...
**The trigger overhaul is COMPLETE** (hit-stamp conditions shipped 2026-07-14, second session — see "Recently completed"). Nothing is pre-briefed for the next session. The queue, in the user's standing priority order:

1. **User testing of the whole 2026-07-14 trigger batch** on the deployed site (team-relative events, REPEAT_UNTIL staging, rich conditions, hit stamps + windows). Deploy spot-checks are listed under both July-14 entries below.
2. **Remaining feature queue** (user picks): projectile linger, hero alternate behavior slots, source-specific vessel triggers.
3. **Pinned for later:** homing-reflect return-leg timing divergence (plan doc §Phase E) — revisit when a puzzle leans on reflect+homing.

**Decided 2026-07-14 (do not relitigate):** BASE-party sensing (charm-blind); same-team events exclude self but SEE stealthed teammates; legacy events map at read time by authoring side (no migration); REPEAT_UNTIL uses SEGMENT semantics (each block loops back only to the previous block); `repeated_times` means "the segment has run N times" and resets on fall-through; counts are a census (stealth doesn't hide); hit stamps ignore invulnerable/deflecting targets but count shield-absorbed blows; AOE splash / DOT / tile / push damage is 'any'-kind only; reflected bolts credit no attacker. Also standing from 2026-07-13: allies collecting items + scoring is intended.
...
Four slices, one delivery kind per commit as pre-briefed, 544 tests green throughout (22 pins in `hit-stamps.test.ts`), corpus untouched, build green.

- **Foundation + melee + windows** ([`703c329`](https://github.com/Jantzulu/puzzle-daily/commit/703c329)) — when damage lands (past invulnerability and deflect; shield absorption still counts), `applyDamageToEntity`/`NoDeflect` stamp the victim's `hitStamps` and the attacker's `dealtStamps` with the turn number under the delivery kind plus `'any'` (`stampHitLanded` in actions.ts). Eight events — `hit_by_melee/projectile/contact/any` + `landed_*_hit` mirrors — read them as pure predicates with the user's three windows (`TriggerConfig.eventWindow` / `CharacterAction.untilWindow`, default `previous_action` = stamp ≥ currentTurn−1; `this_cycle` = stamp ≥ `cycleStartTurn`, refreshed on every REPEAT/REPEAT_UNTIL loop-back; `ever` sticky). Stamps are new-object writes; they ride ALL enemy wrapper copy-backs both directions (action loop, both trigger-phase blocks, REPEAT_UNTIL condition holder), and the actor loops merge feedback-damage stamps off the original via `mergeHitStamps` (per-key latest — same window the externalHealthBefore merges protect). `initializeGameState` strips stamps so `ever` can't leak across runs. Zero-damage combined-lethality calls never stamp.
- **Projectile kind** ([`cf730d6`](https://github.com/Jantzulu/puzzle-daily/commit/cf730d6)) — `applyEntityHit` + `checkHomingPathForHits` (both modes → solver parity pinned) pass `'projectile'`; attacker credited via `findProjectileAttacker` (sourceEnemyIndex first — duplicate same-asset enemies share an enemyId, the append-only index is the instance identity — then characterId, then first-living-by-id). Reflected bolts credit no one, but the returning hit stamps the caster as victim; deflect bounce-back keeps the projectile kind.
