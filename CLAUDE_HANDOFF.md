...
**MAP EDITOR REDESIGN (user-requested 2026-07-14, audited + Phase 0 shipped same day).** The user pivoted to this over the feature queue. Phase 0 (first-class Ally/Vessel tools — 7-tool row) shipped in `6045ff1`; the remaining phases are outlined and user-approved in shape, but Phase 2's visual direction must be designed WITH the user (their workflow: real code in small isolated commits, screenshots between slices, ask for reference games when style is unclear — never mockups).
...
1. **Phase 1 — decomposition, ZERO visual change.** Extract palettes, ActionsPanel, PuzzleInfo sections, modals into `components/editor/map/*`. Pure-refactor commits so later visual slices stay small and revertable. Playtest already mounts `<Game/>` — do not touch it. ⚠️ The file is autosave-critical (localStorage flush on cleanup + beforeunload) — preserve that wiring exactly.
2. **Phase 2 — layout rework (co-design each slice with the user).** Proposal on the table: single right sidebar with TABS — Build (tools+palette) / Rules (win conditions, par, side quests) / Details (name, skin, sounds); Actions collapse into a compact top toolbar (Save/Playtest/Validate/Publish prominent, Export/Import/Clear/Generate in overflow); placed-entity roster (click to locate/remove); status bar (cursor tile, current tool); dungeon-panel theming pass LAST, with user references.
3. **Phase 3 — interaction niceties, user cherry-picks:** right-click remove / eraser, click placed entity → inspect popover (sprite, HP, behavior tooltip, remove), drag-to-move placements.

**Also pending:** user testing of the whole 2026-07-14 batch (trigger overhaul + hit stamps + instant dispel + Phase 0) on the deployed site. Then the feature queue: projectile linger, hero behavior slots, vessel triggers; homing-reflect timing stays pinned (plan doc §Phase E).
