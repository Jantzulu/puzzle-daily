...
Last Updated: August 10, 2026 — **DEATH VIGNETTE SHIPPED (`5a226a1`) + THE QUEST-SCROLL 1-PX MYSTERY SOLVED + KIT v20 BAND BUMP (`ee2ede8`).** (1) The defeat window's 💀 is now the death animation of the last hero who fell that round (the user's girlfriend's idea) — new `DeathSpriteThumbnail` (src/components/game/) wraps the board's EXPORTED `drawDeathSprite` (play once, hold corpse frame) + a port of `drawSoul` (rise/sway/fade); `deathVignetteUsable` gates, skull stays for no-death-art/concede, ⏳ turns branch untouched; last-fallen = `placedCharacters` dead && !despawned, max diedOnTurn. DELIBERATELY a standalone component: the user is holding the Game-over screen for their own design — porting later is one import. Fallback branch verified live headlessly (pane profile has spriteless seed heroes); the real animated branch debuts on the user's sprited heroes. (2) User report while painting the quest scroll: bottom line flat = fine, ANY portion raised 1 art px = "rendering jumps many px". Workflow-verified: NOTHING in slicer/renderer/layout derives numbers from paint (all rects fixed template geometry; questFrameBorders.b feeds exactly one padding, 1:1). Root cause found by decoding their live export: the painted line sat pinned ON THE TOP ROW of the 5-art bottom band — +1px crossed the invisible chunk boundary into the thin-edge GUTTER RING, regions deliberately never cut (rep≥1) — pixels silently discarded, live render fills that strip with the tiled center master (solid parchment) at content-height-dependent phase. The WYSIWYG boundary is invisible while painting; when an artist reports nonlinear art→render behavior, decode the export and probe rows FIRST. (3) User ask, kit v20 (`ee2ede8`, DEFAULTS_VERSION 20): quest-box top/bottom bands 5 → 8 art for scroll-shaping headroom. PROVEN safe ahead of re-exported art: band heights appear in NO other region and NEITHER sheet dimension (assembleNineSliceCapped W/H key off corners + top-cap widths + left-run heights), so the painted sheet re-cuts with NO migration — the cut simply grows into the gutter rows that were eating the line; committed 5-tall JSON keeps rendering (slices JSONs are self-describing). Migration honors the v12 artist-sizes-sacred law: the six band pieces adopt 8 ONLY where stored h is still the old default 5. Assembly invariant tests green; tonight's slice re-export rode along (art-drop sweep). USER NEXT: reload the forge tab (header shows v20), re-download the template (same sheet size, new taller band outlines), paint the scroll shape; after re-export the box gains 6 CSS px padding top+bottom (band = the thinness lever) and the top medallion recenters ~3 px lower on the thicker band; ceiling for future bumps = the 12-art corner height. (4) SAME NIGHT, kit v21 CORNER OVERHANG (user ask: "draw below corner BL/BR and above the top corners, attached to those elements" — the scroll's rod tails): 8-art vertical halo on the four corners (t on the top pair, b on the bottom), cut+shipped like the plate halo; SHEET GROWS 144×157 → 144×173 (8 rows inserted between plate band and frame top + 8 appended below — band heights still feed no other region, but corner paint room must be physically adjacent to its chunk). Renderers went NOMINAL for all corner-keyed geometry (QuestBoxSkin + PanelNineSlice; backward compatible — halo-less JSON reduces to the old math exactly); preview corners escaped the overflow-hidden panel box onto the ground wrapper inside a game-parity clip (8 art past the box = QuestBoxFrame's BLEED), ground pads reserve the halo reach. A 3-lens adversarial review CAUGHT A BLOCKER before commit: the slicer's generic stale-sheet hint says ANCHOR BOTTOM, which for this first-ever mid-sheet growth would pass the size gate and slice garbage silently — KitSpec gained `migrationHint` (kit-specific recipe shown by the hard stop; REQUIRED for any future non-top sheet growth), and exportAssembledTemplate now draws the magenta halo paint boxes (they existed only on-screen; the artist paints against the PNG). ARTIST MIGRATION (in the hard stop + v21 history comment): Canvas +16 ANCHOR TOP, then drag everything from y=37 down by 8. (5) THE USER'S "not pixel perfect" ornament report, root cause: centering math rounded to CSS px — an ODD offset = half an art px off the frame's grid (questPlateStraddle was 17!). Ornament x/y centering + plate straddle now SNAP TO THE ART GRID (multiples of Z; straddle rounds in art px → 16, the plate sits 1 CSS px higher), preview mirrors it. PINNED LAW: any offset between two pieces of panel art must be a multiple of Z — integer CSS px is not enough, the art GRIDS must align. Kit v22 (same arc): plate-mid SPLIT into plate-mid-l/-r occupying EXACTLY the sample's two old mid slots (the discarded rep-1 seam preview now SHIPS) — sheet layout/cuts/caps unchanged, NO migration; band renders BOOKMATCHED (left tiles from left cap, right tiles RIGHT-ANCHORED, overflow crops at the center seam); legacy single-mid exports keep the original single-span tiling (a split would phase-shift committed art — QuestBoxSkin plateSplit gate); kitFamily also keys on plate-mid-l now. (7) THE SCROLL TOGGLE (user ask 2026-08-11, next session): the QUEST seal is a BUTTON — click rolls the scroll up (reverse keyframes on the same stamped vars → un-stamp → box height collapses to the padding floor so HEROES rises) leaving only the seal; click again replays stamp+unfurl via DISTINCT remount keys and the box height-transitions back. HARD-WON: (a) the expansion target must measure the IN-FLOW content + paddings, NEVER box.scrollHeight — scrollHeight counts absolutely-positioned canvas BLEED overhang (ornament canvas = +48px → the box opened "very tall" then snapped, user bug); (b) the frame/content wrappers are HIDDEN at 'closed' (display), NOT conditionally unmounted — keyed conditional siblings left an ORPHANED frame that stacked under the reopen replay (user bug: "animation on top of the already existing one"); distinct key prefixes (quest-frame-/quest-content-) either way; (c) height rides a one-shot 0.3s transition between measured px (null = auto at rest), padding never animates (border-box floors the collapsed height at the padding sum = the seal's shelf). ROUND 2 SAME SESSION (user tightening, all shipped): the seal STAMPS WITH the scroll vertically centered on it (--qseal-start = h/2; rise on the WRAPPERS, stamp scale on the INNER button/ornaments div — two transforms on one element override each other) then rises to its straddle at unfurl start; the closed scroll TUCKS behind the seal instead of un-stamping (.quest-tucked → translateY(-50%) on the always-mounted .quest-frame-layer — the % self-tracks the collapsing box) at its FULL open length (frame layer pins inline height + bottom:auto so the RO never redraws the rods small — "squishing does not make sense if it was a physical scroll"); reopen LOWERS the rolled scroll then unfurls via .quest-reopening (entrance keyframes delay-free; class STAYS at rest — reverting to base entrance animations would re-apply and REPLAY them; same reason phase classes pin animation:none on the stamp host). NO remounts anywhere — replay keys deleted; phases open/reopened→closing(0.5s)→closed(tucked)→lowering(0.35s)→reopening(0.65s)→reopened; button disabled mid-phase; skinned-mode only. AWAITING USER EYES on the ornament fix (shared-ruler law — no visual fix is done until they say so). (6) THE SCROLL-OPENING ENTRANCE (user idea, same night, USER-APPROVED on desktop Chrome: "it looks great!"): QuestBoxFrame split PERMANENTLY into THREE canvases — PAPER (center + top/bottom runs/caps) under two ROLL columns (each side's corners + vertical run, the painted rods) — and the entrance parts the rolls from butted-at-center (the closed scroll, visible from first paint via backwards fill) outward via pure transform, while the paper reveals through a one-shot clip-path wipe whose boundary TRACKS THE ROLLS EXACTLY (inset interpolates half-width → roll width over the same curve as the travel; both linear in progress; probe-verified flush to 0.0001px at mid-flight). Box LAYOUT never animates — the page below holds still. Choreography RE-ORDERED same night (user call): SEAL stamps FIRST @ 0.3s → closed scroll fades in @ 0.75s → rolls part @ 1s with the objective UNFURLING INSIDE THE WIPE (quest-scroll-content: same clock as the paper, ends at inset 0, boundary trails the paper's by the padding offset so text never meets the raw cut — the old naked-wipe rejection doesn't apply behind a moving rod) → ignite + bob @ 1.65s (glow LAST). Clip vars stamped on the ANCHOR (content wrapper is a sibling of the frame layer). INK ON PARCHMENT: objective + (?) are BLACK when the skin is live — the glyph fill is the SHIMMER's --flare-base, not the Tailwind class (.shimmer-ink override; HelpButton needs !important color overrides); baseline keeps copper + plain fade. LAWS LEARNED/APPLIED: (a) clip-path keyframe endpoints must be SAME-UNIT (px→px via stamped --qclose = w/2; a 50%→24px mixed interpolation can fall back to DISCRETE on some engines — paper snaps to full width mid-open; the user's first sighting in the embedded editor pane looked exactly like that); (b) travel/close vars are stamped SYNCHRONOUSLY in the layout effect (the async decode-gated draw would let first paint see the 0px fallbacks); (c) the exact-tracking contract: clip resting insets == roll widths — change one, change both (side gutter strips ride the ROLL canvases so tracking never reveals art under a column). QuestBoxSkin exports values+components = Fast Refresh breaks on its edits — hard-reload before judging (bit again this round: --qclose read empty on a stale module graph). Previous: August 6, 2026 — **THE iOS 26 "LIQUID GLASS" SAGA (`c8c32ea`): Safari 26 clips sticky-layer content at the glass chrome BY DESIGN — the user's accidental iPhone update exposed it, a phone-on-LAN debug loop bounded it, and the closed-gate TAIL now ships DORMANT.** The user's report: the scrolled sticky rail ends in hollow space under the new glass chrome. Their design (right on both diagnosis and fix): build the closed portcullis's bottom rungs above the control rung. Built three ways (out-of-box child span, negative-top anchor, IN-BOX growth of the mesh's own canvas — final form: ONE canvas, no cross-canvas seam, svg baseline untouched via skin-only inline height) + compositor-layer promotion — iOS refused all four, each verified FRESH on the phone via a magenta freshness-marker methodology (getImageData verifies the BACKING STORE, never compositing — that gap burned a round). RESEARCH (4-angle sweep, links in the `c8c32ea` message): known since 26.0, Apple treats it as intended (WebKit 301756: Safari paints its own one-shot-SAMPLED color slab over the glass region when it detects a fixed/sticky element near the edge; roots without explicit background = solid black/white fallback slab — html now carries an explicit background-color, body already did); community recipe (paint-free sticky wrapper + abs-child visuals) already matches our structure and still doesn't admit above-box content; no point release through 26.5 changes the core clip. **NOVEL, UNREPORTED FINDING: on this SPA the slab-vs-translucent mode FLIP-FLOPS per load — Safari's one-shot sticky-detection racing React's mount (sampler first → translucent; React first → slab). Candidate exploit, UNTESTED: defer position:sticky until first scroll so the sampler never sees a sticky element — would stabilize the good mode (not reveal the tail). Also pending: Feedback Assistant report to Apple (the flip-flop is file-worthy).** THE TAIL WAS REMOVED LATER THE SAME DAY: iOS never painted it, the design answer landed instead (user cut: rail IN-FLOW below md — sags 3px at scroll start then travels with the page; sticky+docking unchanged on desktop; mobile docking works because the hamburger is only reachable near page top where in-flow = the old sticky rest position), and once docked the tail's rungs showed through the open menu's lattice gaps as DOUBLED IRON (user catch; settle depth pulled to 24px — within the mesh's 27.5px bar reach). The un-stick arc also: unified gate open/close onto the settle's iron curve, then PERCEPTION-tuned (close = winch-in curve at 1.1s vs open 1.4s — equal clocks read unequal because the close's tail exits offscreen; four live rounds); load settle = 32px→24px start, 0.35s delay, balanced curve; LOGO click now dismisses the open gate like the rung links (user bug). TOOLING BORN OF THE SAGA: `dev-lan` launch config (vite --host, port 5184) = the phone-on-LAN loop, seconds per round vs Netlify builds — use it for every future device-specific hunt. Earlier same day (all pushed): corrected hearts; PLAY/CONCEDE label seat (1px); LIVES/TURNS label −1px; active-menu plate glow (canvas drop-shadow) + the TEXT glow rectangle saga (glyph blur boxes over canvas plates on mobile Safari — THIRD strike of the no-text-shadow-blur-over-art law; now a gradient ::after bloom between plate and glyphs, label z-1) + both glows strengthened; quest box w-fit + mx-auto + trimmed vertical padding (the JSX comment-in-expression trap struck its THIRD time en route — Babel 500s while tsc stays green); the user's quest art re-export (was sitting uncommitted through a push — SWEEP git status FOR ART DROPS BEFORE EVERY PUSH). Previous: August 5, 2026 (night close) — **THE RAIL ARC SHIPPED AND PUSHED THROUGH THE GEM-DYNAMICS ROUND; user signed off and called it a night.** The tail rounds after the concede-art fix (all pushed): plaques went STONE-ANCHORED (`467841a` — cell-centering made the stone gap scale with viewport, ~85px adrift on desktop vs right on mobile; now a constant 2px off the frame's 16px aura box at every width); the PLAY label dims WITH the face (`f551d56`, same PLAY_STONE_DIM + 0.25s crossfade; safe — opacity guards painted layers, not DOM text); the painted gems gained TWO compositor-only dynamics (`182c151` halved by `380915c`, user: too intense): a scroll SHINE band masked by the face png itself (mask static, transform-only) + a BREATH twin (brightStoneFace = dim's sibling, warm wash, opacity keyframe) — play face only, never while dimmed, concede deliberately still; the shine's travel then went through two calibration rounds (`854d817` brief-sweep, WRONG) landing at `af9e923`: **THE MESH GEMS' EXACT PHYSICS scaled to the stone (user's model: "a crystal moving past a FIXED light") — 228px/unit = 380 viewBox units on the 200-unit mesh gem; band geometry starts off-stone so rest = hidden for free; partial scroll = partial sweep.** ANSWERED, NOT WIRED (user: "not right now"): plaque-kit AURA — the kit cuts are exact-nominal everywhere today; the header RENDERER is already halo-ready (4 art/side via its 8px bleed) but assemblePlaque ships no halos, and the MAIN plaque renderer is bitmap-anchored/bleed-0 (needs nomW + bleed + preview lockstep if ever wired); the plaque sheet was still placeholder when answered — **and then the user painted it THE SAME NIGHT: the 4:26 AM art drop (committed at session close) painted ALL SIX rung-plaque pieces (main 3-slice + headers) plus fresh control-rail / nav-gate / portcullis-gate exports — EVERY RAIL SURFACE IS THE USER'S ART now (adding plaque aura later = kit bump + their sheet migration, no longer free). Only the avatar-crest remains unpainted (empty in the nav-gate export, graceful fallback).** Previous entry: — **THE RAIL REDESIGN IS TUNED, PARTLY PAINTED, AND USER-APPROVED ("Looks so good!"); THE CONCEDE TRIO IS THE USER'S ART, LIVE.** Tonight's rounds on top of the v19 base (all committed, NOT pushed): (1) TWEAK ROUNDS `71bee78`+`6665d8a`: plaques center between rung END and stone ART edge (grid px-3 dropped; per-cell 16px aura pad) with a two-round 8px inward bias; LIVES/TURNS labels deterministically ink-centered on their plates (the wrapper was flex-centering the rail's inherited 20px strut — killed via fontSize/lineHeight 0 + measured ink offset, marginTop = h/2 − 6) and shrunk to 10px inline (scoped-class trap; PlaqueHeaderLabel = the exported single register); hearts + counter share a +2 drop (`238e039`, tuned 3→2 live) clearing the header plates. (2) HEARTS HARDCODED `fd51236`: the user's 7×7 paintings at native ×2 (src/assets/icons/heart-*.png; lost life = its own painting, no opacity dim); iconHeart/iconHeartEmpty settings DELETED (only consumer) + dead renderLivesHearts removed. (3) **STALE-SKIN TRAP CLOSED `f6ddef5`**: JSON drops could leave tabs on the old module graph (Vite: 'Could not Fast Refresh — dimStoneFace export incompatible'; bit the user: a perfect export "didn't even dim") — a vite plugin now FULL-RELOADS on any src/assets/panels/*.json change, verified live. (4) **THE OPAQUE-FRAME LESSON `42d578e`→`6306b96`**: the user's first concede export painted a FULL STONE into the frame slot (1080/1080 face px opaque vs the ring's 60) — the frame draws last over every state and never dims, so it hid dim/hover/pressed/concede while every piece looked perfect; diagnosed by pixel-probing the three stone canvases (base bright / twin dark / FRAME opaque green), repaired from git, and superseded by the user's own re-export from their fixed sheet (ring true, verified). WHEN STATES "DON'T WORK" ON A PAINTED STONE, PROBE THE FRAME FIRST. (5) Unit-centering quick test (whole LIVES header+plaque centered on the rung) tried and REVERTED — user: "not a fan"; the settled look keeps the main plaque rung-centered with the header proud. STILL PLACEHOLDER: rung-plaque main+header art, avatar-crest. Previous entry (the v19 redesign itself, its laws and commit map) follows: — **THE RAIL REDESIGN (v19) IS LIVE ON PLACEHOLDERS, AWAITING THE USER'S TWEAK ROUND BEFORE THEY PAINT.** User spec, their words: plaques become mini quest-boxes (small header plate riding proud with LIVES/TURNS, content alone in the main box), the TURNS plaque owns the run counter (0/X in setup → counting live), the Play stone BECOMES the Concede button mid-run ("share the frame but have the same 3 states"), and all rail labels go caps/bold. Baseline tag `pre-plaque-redesign` (= `3aadff9`, the first plaque placeholder commit) is the revert point for the whole experiment. Five commits, each verified live before the next: `3aadff9` rung-plaque placeholder art (System.Drawing-generated, the artist drop path, pixel-probed); `44666a3` kit v19 + plumbing (game-inert): rung-plaque gains header-cap-l/mid/cap-r (4/8/4 × 9 art — ids deliberately NOT plate-*, that flips kitFamily to quest-box; artist migration = Canvas Size +11 ANCHOR BOTTOM), play-gem gains play-button-concede/-hover/-pressed (60×18, appended at the sheet's RIGHT — Canvas Size +210 ANCHOR LEFT; unpainted concede = procedural topaz fallback, NEVER borrows play faces), RungPlaque header machinery (PlaqueHeader + drawPlaqueHeader, parameterized, 60%-proud straddle = the quest rule at plaque scale, hud-label copper register = the QUEST plate's recipe, inline-chip fallback when header art absent), PlayStoneSkin face='play'|'concede' under the ONE shared never-dimming frame, PortcullisLive plaque+stone previews updated same-commit (ships-with-preview law), useCrispSnap defer widened to a FRAME COUNT (header = snapped child inside RungPlaque's already-deferred snap → 3 frames), placeholder art appended to BOTH slices JSONs with the user's painted play-gem pieces verified byte-exact on rewrite (PS 5.1 trap closed: ConvertFrom-Json parses cutAt into DateTime — carry the original string); `1f8ef5f` LIVES header live (11px straddle, integer-aligned, pixel-probed); `535dc19` TURNS plaque owns the counter in EVERY rail state + center stone concedes: warning colors carried (rust ≤3, blood+pulse ≤1, no glow), unlimited puzzles show the bare count, plaque width FIXED at the counter's widest state per user mid-arc call — invisible ghost of max/max measured in the REAL themed font reserves it (ghost must be inline-flex items-baseline, NOT a block flex item: it inherits the theme root's 30px line box and inflated the plaque to 30px, caught live), green gem-plate-aura DROPPED from the concede state (activated breath behind a destructive control read wrong; the counting plaque signals run-in-progress), old right-third Concede deleted, center column carries inline zIndex 2 (user report: the later-sibling right plaque painted OVER the Play button — .control-rail > * z:1 outranks utility classes, so the fix must be inline; the action stone wins overlaps now); `ee51c01` PLAY/CONCEDE uppercase + tracking-[0.05em] at the button's own size (CONCEDE = 72px of tracked caps in Modern Antiqua, measured live in the themed face per the pinned trap). Verified end-to-end in the live browser: setup 0/100 → place → Play → CONCEDE stone + ticking counter at the SAME 78.125px plaque width → confirm dialog → cancel; a natural defeat exercised defeat→setup. Mobile 375 clean — the old "Max Turns:" two-line wrap died with the redesign. NOT pushed. **NEXT: the user eyeballs the placeholder geometry (header register 9 art / straddle 60% / counter format N/X are all one-line tunables) and directs tweaks; then paints rung-plaque (main + header) and the concede trio; avatar-crest still placeholder too.** ——— Previous top block: **EVERY FORGE KIT NOW WEARS THE USER'S PAINTED ART: quest box, control rail, gate bars, nav menu (beams/caps/bars/plates/signs), utility pills, and the Play Stone (face+hover+pressed+frame).** The giant entry below this line is the CONTIGUOUS Aug 1–4 painting-arc log, appended chronologically as each round closed — read top-to-bottom for: pipeline build → kit audits (DEFAULTS_VERSION now 17) → the reset trap + sizes-preserving migrations → aura/inset/centering rounds → the corner-pixel and dark-bars sagas (method lessons: shared ruler, user-as-judge, A/B layer toggles, opaque pixel-baked dims, y-only snaps, every kit family ships WITH its live preview) → pill plates + avatar crest → Play Stone. Next: the user keeps painting/iterating art; sprite-bust avatar pipeline captured in docs/feature-backlog.md. ——— The log begins: August 1, 2026 (sixth push) — **EVERY PORTCULLIS SURFACE NOW HAS A FORGE→GAME DROP PATH (`41a0675`, pushed), geometry AUDITED BEFORE PAINT per the user's explicit ask ("make sure the guide represents the actual geometry before I draw anything").** Three kits, three committed placeholder files, three canvas renderers — the quest box pipeline generalized. **The artist loop per surface: paint the kit in the Panel Forge → Export cut pieces JSON → overwrite `src/assets/panels/<kit-id>-slices.json` (`portcullis-gate` / `control-rail` / `nav-gate`) → HMR → the live page wears it; empty placeholder = the procedural SVG renders byte-identically (round-trip verified).** (1) **`panelSkins.ts` (new)** — the shared plumbing extracted from QuestBoxSkin: Z=2, hardened `buildSkin` (kitId guard, null-safe, size/halo bounds), device-res canvas prep, phase-anchored tiling, decode gating, `useCrispSnap`, and `BAR_FRACS` (the six bar fractions both meshes share — change only in lockstep with the svg BAR_XS lists). (2) **KITS CORRECTED TO AUDITED LIVE GEOMETRY, DEFAULTS_VERSION 7** — the old kits did NOT match the meshes: beam-face 12→**18** art (the settled thin 36px iron), sign pieces 10→**13** (the ~26px CSS plates), bars 5→**6** wide (mobile-true ~12px), rail band 16→**22** (fills the 44px rung box), spikes 7→**8**; each kit description now states its wired drop path, the audit notes, and the caveat that the SVG meshes STRETCH with the viewport while art is fixed-size — art matches mobile truth, desktop reads slightly narrower bars. Forge plates: kit 4 art = 8px vs the meshes' 9px CSS (quantization, noted in the kit; resize to 5 for 10px if preferred). (3) **Renderers, all inside the existing mesh components so every call site works untouched:** `PortcullisSkin` (PortcullisMesh.tsx) draws into a div carrying the mesh's own className — `.control-rail-mesh` AND the menu's `.nav-gate-rail-mesh` variant both place it — band fixed 22 art at bandTop=round(31.25% of the box) on the rung zone, bars rising at BAR_FRACS, plates +13, spikes hanging below; `GateBeamSkin` (GateMesh.tsx) uses canvas BLEED to bridge item gaps exactly like the svg overhang trick (first item reaches 26px up behind the navbar, every beam sends bar tips 12px down for the next sibling to paint over); `GateSign` = 3-slice sign canvas behind the DOM label (App/PlayerApp nav spans swapped to it; skinned mode kills the CSS plate stock, ::before/::after bolts, AND the fractional translateY(-1.8px) — integer-px law), self-snapping deferred, no negative z (pinned WebKit rule). **NOT skinned: the `.nav-pill` utility buttons (sound/cloud/profile) — only the five gate rows; a `nav-pill` kit is a possible follow-up if the user wants them in art too.** (4) **rAF LESSON, applies to ALL skin canvases: draws are SYNCHRONOUS now** — schedule-via-requestAnimationFrame left canvases BLANK in hidden/background tabs (rAF frozen; caught live when the freshly-wired rail rendered nothing), and RO callbacks already batch. (5) Verified end-to-end with synthetic solid-color payloads generated via System.Drawing in PowerShell (NEVER hand-transcribe base64 — a manual copy corrupted a header first try): every zone pixel-probed at audited coordinates on all three surfaces, then placeholders restored and every SVG returned. Gates: tsc -b clean, lint 0 errors, 678/678. **NEXT: the user paints the three kits (quest box first proved the loop).** **FOLLOW-UP `9a3bc81` (same day, seventh push) — THE FLUSH OUTER PAIR + DETACHED HARDWARE CUTS, triggered by the user asking whether the kits handle the portcullis's OUTSIDE bars/spikes (they only half did): (a) `panelSkins.barCenters(w, barW)` clamps the outer bar pair FLUSH at the edges in both skin renderers AND both forge assemblers (fraction-centered fixed art drifted ~4px inward at 672px; the svgs were flush for free because half-width and center share the viewBox fraction) — plates/spikes ride the same centers, edge spikes clip at the canvas edge like the mesh's viewBox clip. (b) A 3-lens adversarial review (9/9 findings CONFIRMED) then proved the in-band flush hardware was baking paint into OTHER pieces' cut regions — ONE FLAT SHEET CANNOT SHIP THE SAME PIXELS IN TWO PIECES. Worst case: plates painted per guides + caps left unpainted (per the cap note!) exported caps as "painted" holding a floating plate → 16px transparent HOLES in the live band. Also confirmed pre-existing in the nav kit: the bar's cut sat under the beam row (18 of 24 rows shipped beam paint — no clean bar span exists in the lattice). FIX = the quest-plate pattern generalized: `AssemblyRegion.guide` (dashed placement previews the slicer wholly ignores — never cut/compared/counted), plates + the nav bar CUT from detached slots on clean ground above the sample, in-band positions render as dashed guides (notes say leave hardware guides unpainted), sign stays painted in place (opaque over iron = live layering, sits on beam REPEATS never the cut period). Skin plates moved 13→14px below the iron top (even = art-grid aligned; sample/preview/live agree exactly). DEFAULTS_VERSION 9. NEW TEST `src/components/editor/__tests__/panelForgeAssembly.test.ts` pins the whole class for EVERY built-in kit: cut regions full-size + on-sheet (halo included) AND nothing later-drawn may paint over any cut region. 683 tests. LAW FOR FUTURE KITS: hardware that lives ON another piece's paint gets a detached cut slot + guide regions, ALWAYS.** **QUEST ORNAMENT ROUND (`146c03e` + `0c6b937`, user painting live): (1) edge medallions are now the TOPMOST ART LAYER — they rendered under the QUEST plate (a centered top medallion is exactly where the plate is) and the forge preview's overflow-hidden panel box clipped their halo overhang; game = new `QuestOrnaments` bleed canvas mounted after the plate + before the content wrapper (plate label carries z-1 — DOM text still above ALL art), preview = ornament layers on the ground wrapper after the plate with sample content z-1. (2) Ornaments 12→17 art tall (user: ~5px more drawing room), and assembleQuestBox's band now sizes to the TALLEST paint box in it with ornament slots centered — ornament resizes grow the sheet instead of shoving the halo off it. v10 SUPERSEDED SAME NIGHT by **`2d45d00` v11** — the user's exact spec, in their words: "expand the template by 5 pixels on the top of it — anchored on bottom, only expand up — and accept this new space as AURA for the ornaments." Ornament pieces back to 24×12; the sheet gains 5 top rows, everything else keeps its layout (Aseprite migration = Canvas Size +5 anchor bottom, paint doesn't move); ornament slots' upward halo = 13 art and the new rows are theirs alone. QuestOrnaments got its own 48-CSS bleed (16 was silently clipping tall overhang — aura reaches ~38 CSS past the box on thin bands); preview ground pad reserves the reach. ⚠️ THE RESET TRAP FIRED LIVE AND IS NOW CLOSED (`27a1b96` v12): tonight's v8–v11 bumps wholesale-reset stored built-ins, wiping the user's thinned quest sizes — and since kit assembly regions ARE the slice map, re-slicing their painted sheet with the reset map SHREDDED the Live panel into misplaced fragments (user: "it seems completely broken"). Fixes: (1) THE SHIPPED ART IS THE SPEC — built-in quest defaults now mirror the committed slices file (top/bottom bands 5 art); (2) THE ARTIST'S SIZES ARE SACRED — from v12 on, migrations carry stored built-in piece w/h over and refresh only descriptions/notes/new pieces (pre-v12 copies get one final wholesale replace onto the art-true defaults). Future size corrections ship as NEW piece ids or re-apply notes — never as silent default changes. **`d022bfe` v13 (rail round, user painting): rail-spike cut carries ±8 art SIDE aura (height exact by user call — "I don't need them made taller"); rail caps VERTICALLY CENTERED on the band in template + live renderer (were top-aligned; template face row drops by the overhang); PortcullisSkin hardware now anchors by NOMINAL with halo offsets. First notes-only refresh through the v12 sizes-preserving migration. Spike-size truth told to the user: kit spikes are mobile-true — the svg mesh stretches spikes to ~30 CSS wide at the 672px desktop rail vs the kit's fixed 16 (height matches); user chose to LEAVE spike size for now. Plates lean the other way (8 CSS art vs mesh's 9 desktop / ~5 mobile). Painted art NEVER stretches with viewport — that's the fixed-art trade. **`0514b5f` PORTCULLIS LIVE PANEL: the forge's Live panel only knew nine-slice anatomy, so portcullis kits previewed as "nothing painted" (user hit it painting the rail). The game's skin draws are now EXPORTED routines — drawRailSkin (PortcullisMesh) / drawGateBeamSkin + drawGateSignSkin (GateMesh) — and editor/PortcullisLive.tsx feeds them fresh slices via panelSkins.skinFromSlices (same validation as the committed path): rail/gate kits preview on the live 88px rail box (dashed 44px rung guide, partner-kit note), nav kit as two stacked items + sign plate behind a DOM label, W slider with Phone@393/Desktop@672 presets. SHARE-THE-RENDERER LAW: never write a parallel preview implementation — export the game's draw and feed it. Game surfaces regression-verified post-extraction; forge page login-gated headlessly (user's paint session = its visual gate); panelSkins.test.ts pins the adapter. `e88be3e`: FORGE PLATES vertically centred on the rail band (renderer + template guides — the old art-y-7 line was the svg's bar-crossing height, 2 art above the 22-art band's centre; the user's earlier "railcaps centred" ask, it turns out, meant these plates AND round two caught the plates still off). Beam plates locked to the same centred rule (pixel-identical at 18-art iron). Pixel-verified: plate rows 18–25 of the 44 CSS band. `86b1a65` LATTICE INSET (user call): outer bars/spikes 2 art in from flush (EDGE_BAR_INSET in panelSkins), interior four EVENLY spaced between them (fraction interior was off-pitch post-clamp) — one barCenters change moves rail + menu + plates + spikes + both templates in lockstep; default outer spikes now fit unclipped; svg baselines untouched (transitional 2-art offset vs painted neighbours until art replaces all). Plus: PortcullisLive's rung guide is a toggleable integer-row OVERLAY — its corner dashes showing through cap-art transparency were the user's "odd single extra corner pixels" (their art was fine), and after a SECOND report `ca5ea1c` defaulted it OFF: a mimic repro (stepped rounded caps, triangular aura spikes) pixel-probed every cutaway zone CLEAN — the renderer never paints behind cap cutaways; the remaining marks are the guide (opt-in now), the sheet's own top-left detached plate slot (intentional), and the outer spikes' roots sitting close to the corner rounding at the 2-art inset (the user's art at the user's position — tunable, not a bug). `7413db1` v14: the gate kit's BAR-TOP-CAP cut DETACHED to clean ground (top-left slot + on-bar dashed guides) — its cut was the top rows of the first bar column, so a naturally painted full-length shaft leaked its top rows into the cap piece ("top cap has extra pixels"; the plate law's disease, second occurrence this arc). Gate sheet +6 rows; artist migration = Canvas Size +6 anchor bottom, then re-export the template. THE "EXTRA CORNER PIXELS" SAGA'S FULL LEDGER (three sightings, three different causes, user's art innocent every time): (1+2) the Live panel's rung-guide overlay through cap cutaways (now opt-in), (3) `8c058da` the piece strip's FIXED 8px transparency checker — one cell dwarfed a 6×4 cap's corner notch and read as a stray grey pixel; the checker now scales with zoom (2-art-px cells, Aseprite convention). Also shipped en route: `d616364` slicer HARD-STOPS on layout-mismatched sheets with the exact Canvas-Size migration hint (was warn-and-cut-garbage); `a78c980` forge header shows v{DEFAULTS_VERSION} as a STALE-TAB detector (an open SPA tab keeps its old bundle and slices from old coordinates); scan of the user's committed rail export found ZERO template-tint contamination (their layer discipline is clean). LESSON: when the artist reports phantom pixels, suspect every OVERLAY and BACKDROP between them and the art before suspecting the art or the cuts. **MOBILE H-SCROLL BUG (user phone report, fixed `90c71e9`+`3f808f0`): skin-canvas BLEED widens the document — quest ornament canvas (48px aura bleed) overflows the viewport ON FIRST LOAD (right edge 403 @ 375vw, pixel-confirmed), menu beam canvases (32px) after first hamburger open/close (closed gate stays mounted; open-state scroll lock hides it) — the page could pan right and lock there. Fix: `overflow-x: hidden`+`clip` on BOTH html AND body (iOS honors body where it ignores html); the page never scrolls horizontally by design, art overhang rendering untouched, wide dev tables have their own scroll wraps. PWA NOTE: the installed app's service worker serves the old bundle for a load or two — judge fixes only after a close-and-reopen or two reloads. `30b2f82` v15: nav-gate gains BEAM END CAPS (beam-cap-l/r 8×18 fixed — user catch: the tiling face repeated its painted outer outline at every seam; finished ends must be fixed pieces, the rail's rule). Cuts join the EXISTING detached slot row + dashed guides at row ends — sheet size and all existing paint positions unchanged, no migration. drawGateBeamSkin: face tiles between cap nominal edges, caps centered on the iron; unpainted caps = edge-to-edge as before. User's rail + gate-bar + quest-aura art all LIVE. **THE DARK-BARS SAGA (`e1d58bd`, user-verified live): the rising bars' last px above the rung rendered darker since the SVG ERA. Five failed theories (svg/paint mismatch, board bleed-through, scrim ×2, half-pixel) — every canvas/column/piece probed clean — until a LABELED PIXEL RULER in the shared preview + the user's structural instinct ("those pixels act like the rung's, the ones above like the menu's") + an A/B filter toggle convicted `.nav-gate-item`'s `filter: drop-shadow` on the beam layer: per-pixel, the beams' descending bar TIPS cast shadows onto the rail's bars below. FIX: bars on their own UNFILTERED layer (`drawGateBars` → `.nav-gate-bars-skin` z:0; iron+caps+plates keep the shadow on `.nav-gate-beam-skin`; PortcullisLive composes both). Scrim RESTORED verbatim (exonerated); bar-top-cap no longer drawn on the rail's never-ending bars; rail mesh floor-snaps to whole px (its -62.5% anchor = built-in half-pixel; band now seats exactly on the rung); nav art committed — ALL FOUR SURFACES WEAR ART. METHOD LESSONS, PAID FOR: (1) my screenshots repeatedly read as "fixed" while the user's eyes said no — for subtle visual bugs, sync on a shared labeled ruler and make THE USER the instrument, A/B-toggling one layer at a time; (2) never declare a visual fix without their sign-off; (3) suspect per-pixel FILTERS wherever two canvases hand off a continuous shape. `351e964`: utility pills (sound, Sign In, logged-in profile) wear the sign plates via NavPillSign + .nav-pill-skinned at the menu signs' 26px (avatar 28→24 inside the plate); logged-in state unverifiable headlessly (login expired) — mechanics identical to the verified Sign In. `b9e7f17`: CloudSyncButton pill skinned too (user: "might as well make it look nice for us too") — EVERY nav pill now rides the sign plates; cloud + profile states verified by construction only (creator-gated, headless login expired) — user eyeballs them signed in. `1ed6d66` v16 AVATAR CREST: the smooth CSS avatar circle broke the gate's square-hardware language on the profile plate (user, signed-in test: misaligned + bleeds even centered — "overhang is fine but it should be something I DESIGN with slices, a picture frame or crest attached to the plate"). nav-gate gains 'avatar-crest' (16×16 fixed, 4-art halo all sides, detached slot, sheet unchanged); AvatarCrest (GateMesh) renders it nominal-centered behind the avatar chip; pill content flex-centers (GateSign's inline span had baseline-sat the avatar LOW). Unpainted crest = current look. FUTURE (backlog, user spec): sprite-bust avatar pipeline — player icons cut as busts from PUBLISHED heroes/enemies/ALLIES via a dev position-cut-name tool; emoji are placeholders until then. **`0c428a5` v17 PLAY STONE (next paint target, user-designed): the action button UNIFIED to 120×36px everywhere (60×18 art — superseded the 118/132 split so both pieces are single FIXED paintings, zero slicing; 9-slice measured unnecessary: height never varied). SPLIT: play-button face AUTO-DIMS (0.45, face canvas only) until a hero is placed; play-frame NEVER dims, drawn over the face, 8-art aura for ornate overhang; optional hover/pressed faces swap by CSS under :hover/:active while playable. No gem assumptions (transparency = silhouette; "not a gem" is free). The face doubles as the running Turn-counter plate (lockstep 120px width — keep them matched). Drop: src/assets/panels/play-gem-slices.json; baseline = emerald GemMesh; theme action-button IMAGES still win above both. Other gem-tone buttons (Test/amethyst/ruby/topaz) remain mesh — future kit extension if wanted. Synthetic-verified live incl. frame-immune dim + aura overhang. `5734824` rail-mesh snap is Y-ONLY now: the x-snap (from the dark-bars round) shifted the art 0.5–1px left of the DOM controls + menu lattice at half-pixel-centered viewports (user caught plates off-center of Play; user-confirmed aligned after) — PINNED: layers sharing a fractionally-positioned column must never be snapped individually; snap the shared wrapper or nothing. `397527b`: Play Stone LIVE PREVIEW in the forge (the gem family fell through to PanelNineSlice → "nothing painted" after a good slice — third sighting of the missing-preview class; EVERY new kit family needs its Live panel wired the day it ships). PortcullisLive 'stone' family: real 120×36 composite via the game's exported drawStonePiece, dim toggle + base/hover/pressed picker, W slider hidden. `92a8a86`: THE USER'S PAINTED PLAY STONE IS LIVE (face + hover + pressed + frame — every kit now wears art), and the dim rebuilt OPAQUE after their catch (CSS-opacity dim made the face translucent, plates ghosting through): the waiting state crossfades two fully-opaque layers — bright face + a pixel-darkened twin via dimStoneFace ('source-atop' black wash clipped to the painted silhouette). RULE: state dims on painted art are baked into pixels, never element opacity. `e4f72a7` v18 RUNG PLAQUES (user ask): Lives + Max Turns leave the bare rung for ONE shared painted 3-slice (rung-plaque kit, 13-art register = the sign plates') worn by both at content-driven widths; RungPlaque component + drawRungPlaque (shared with the PortcullisLive 'plaque' preview, which shipped WITH the family); drop = src/assets/panels/rung-plaque-slices.json; the running-state Concede button keeps its own look for now. NEXT PAINT TARGETS: rung-plaque + avatar-crest (both placeholder), plus optional play-button-running if the user wants an engaged run-state face.** Previous: August 1, 2026 (fifth push, small hours) — **THE FORGE→GAME BRIDGE EXISTS (`6b622b2`): the quest box is the first real surface wearing forge-cut art.** HARDCODED BY USER DECISION (they raised it themselves — "does an element like this make more sense to be hardcoded?"; yes: core chrome succeeds BannerMesh as SOURCE, not a theme knob; git is the custodian, both sites get it on deploy, no theme push). **The artist loop: paint (watch loop) → Export cut pieces (JSON) → overwrite `src/assets/panels/quest-box-slices.json` → Vite HMR → the real play page wears it; commit when happy. The checked-in placeholder (empty pieces, kitId quest-box) IS the baseline CSS box; CLEAR = revert that file.** QuestBoxSkin.tsx mirrors PanelNineSlice at Z=1: frame nine-slice + plate 3-slice with the DOM QUEST label ON TOP (user call) + halo overhang + painted divider replacing the side-quests border-t. A 10-agent adversarial review before commit found 4 real holes, all fixed in the same commit: silent divider drop, NaN-cascading halo/w-h validation, missing kitId guard (wrong kit's export → baseline + console.warn), and bare-`null` JSON white-screening BOTH apps at module eval (reproduced through Vite, then guarded with one optional chain). A briefly-added `questBoxSkin` theme key was reverted — do not reintroduce; the decision record is in QuestBoxSkin.tsx's header. NEXT for this thread: the user paints; open question WHEN ART LANDS: Z stays 1? plate DOM text vs taller art plates? entrance animation. **`f041786` mobile-test fixes (user's remote-control report): open menu gate never hides behind the quest box (body.menu-gate-lowered drops the box to z-30 — the z-45 tuck is closed-gate-only now); Directions column sizes to its caption (10px inline fonts — scoped hud-label outranks utilities; pill = caption width, 44px tap height kept); both tap-for-info hint rows UNMOUNT when hidden (the min-h reservation's dead gap read worse than the shift it prevented). **`cee2102` close-side fix:** the lowered class comes off at close-START, so the box's z-restore now rides a `transition: z-index 0s 1s` hold (base state) matching the rail's winch-up — open drops instantly, close holds z-30 the full second, menu-gate-instant snaps. TESTING NOTE: a HIDDEN Browser pane freezes animation timelines (rAF throttled to zero, CSSTransition currentTime stuck at 0) — wall-clock sampling of transitions lies there; seek the animation clock (`anim.currentTime = N`) for deterministic verification. **`fddd714` THE FIRST PAINTED SKIN IS LIVE at Z=2:** the user painted, exported, dropped the JSON — and the game showed it at HALF the scale the forge preview approves at (preview ~2×, renderer was Z=1). Z=2 now; padding/straddle follow. Their painted slices file is committed as the live skin (clear = restore the empty-pieces placeholder from the bridge commit `6b622b2`). RULE: the forge preview's zoom is the artist's reference — keep renderer Z matched to it. Mobile cost noted: ~271px quest text width vs the CSS box's 335 (thick frame's price). **`53093c4` round 2: (a) the "two dark lines beside QUEST" were SUB-PIXEL SEAMS — fractional label width put the plate's cap|mid boundaries on half-pixels and three separately-rastered pixelated backgrounds opened 1px of page at both seams; the plate mid is now ONE continuous band tucked 2px under the caps (seams can only expose art). PATTERN: any multi-piece pixel-art composite with content-driven widths needs a continuous underlayer. (b) Kit v6: edge-top/bottom-ornament — fixed halo'd CENTERED medallions (never stretch/tile), cut from the plate band's flanks, rendered in game + forge preview. (c) THINNESS LEVER: content padding keys off EDGE BANDS (geometry stays corner-keyed) — the user thins edge pieces in the forge (h 12→~5) and repaints to land the box back near the 46px CSS baseline, corners standing proud. Assembler pinned by a scratch test (run + deleted). Preview login EXPIRED on the fresh dev-alt instance — forge page unverifiable headlessly this round; the user's paint session is the visual gate. **`590f83b` thin-frame geometry (three live rounds with the user): PINNED CONTRACT — CORNER-ART TRANSPARENCY IS THE PANEL'S SILHOUETTE; nothing may paint inside a corner square (an underlap "fix" turned stepped corners into rectangles). Final shape in BOTH renderers: runs partition between corner squares; center = notched CROSS (corner-anchored field + gutter strips reaching thin bands, stopping at corner squares, rendered only where band < corner); assembler grew matching rep>=1 gutter regions (rep-0 cut stays clean). The user's thin-band skin is the committed art. **`a2518c9` CAPS-DROP RULE (the user spotted the mechanism: fine when tall, spills when short): a run's two fixed caps need corner-to-corner room — side caps 24+24 under 24px corners = 96px minimum vs the ~54px thin box — so unfit caps ZERO OUT and the middle tiles corner-to-corner; preview computes from props, QuestBoxFrame self-measures (ResizeObserver). The nine-slice's implicit min-size is now handled, not violated. **`26cb81d` mobile artifacts: gutter strips PHASE-LOCKED to the field's tile anchor via backgroundPosition (own-origin tiling banded shaded art at seams) + 2px identical-pixel overlap; `.quest-skin-px` class (not inline) so -webkit-optimize-contrast can cascade under pixelated for older iOS; text-shadow-dungeon is BASELINE-ONLY — the user themselves diagnosed the center 'blur' as the glyph shadow smudging the art. HMR NOTE: QuestBoxFrame holds measured dims in state — stale HMR kept an old closure and 'lost' the bottom strip until a hard reload; hard-reload before diagnosing skin geometry. **`3cedf2b`+`f163327` THE PERSISTENT BLUR, root-caused via the user's "perfect rectangle around QUEST" observation: FRACTIONAL GEOMETRY EVERYWHERE — flex/mx-auto centering half-pixels the box (desktop x=302.5), translateX(-50%) of the text-driven plate width fractional-offsets a possibly-composited layer (the rectangle = the plate's own bounds resampling), and leading-snug × 13px = 17.875px lines fractionalized the mobile box height. Fixes: integer per-breakpoint line heights (18/22/25) + integer-px (?) alignment; `useCrispSnap` (exported from QuestBoxSkin) measures and compensates via POSITION:RELATIVE left/top — never transform, which is what creates the resample path; child snaps DEFER a frame behind ancestor snaps (they otherwise measure pre-snap positions and stay stale by the ancestor's dx). PINNED: bitmap-art elements must land on integer CSS px, and centering-by-transform is FORBIDDEN for them. **`1e3b9df` THE ESCALATION FIRED — CANVAS COMPOSITOR: softness persisted ONLY near text (user's observation — compositor layerization around text-bearing elements resamples overlapping slices; engine-specific, unpinnable remotely), so QuestBoxFrame/QuestPlate/QuestDivider now draw into single device-resolution canvases (integer dpr, smoothing off, BLEED margins for halo overhang) — the board's discipline. Canvas pixels are immune to text layers by construction; single surface also structurally retires seam + phase bug classes. All pinned geometry carried over verbatim; DOM text rides above. THE USER CONFIRMED THE BLUR FIXED. `1aa3dec` cleaned the one canvas-port regression (band drawn full-width under the caps surfaced grey pixels in their transparent rounded-end cutaways — band now spans exactly between cap nominal edges; CAP-ART TRANSPARENCY IS SILHOUETTE, same law as corners). The skin pipeline is now stable end-to-end: paint → export → drop JSON → canvas-composited, crisp on device.** **`86beabb` column round 2 (user, testing with real sentences): Directions column HARD 84px, caption wraps two lines, unset pill says PICK (the card chip's word — CHOOSE overflowed 16px in the themed Modern Antiqua face; ESTIMATE WIDTHS IN THE THEMED FONT, not system fonts), set pill = arrow + compass INITIALS (BEARING_INITIALS export in DirectionPicker.tsx; full word in title + picker readout) with the chevron dropped from the set state only.** Previous: August 1, 2026 (fourth push, late) — **THE QUEST BOX IS THE DESIGN DIRECTION (user: "this design is really solid") + ITS FORGE KIT EXISTS.** The user's plate-box mock replaced the cloth banner (`e8bad8a` rough shape, `70e3ed1` round 2, `ec1468c` round 3): dark bordered box, QUEST plate riding the top border TRUE-CENTERED (a plate+(?) flex group skewed it — the (?) is back inline before the quest text), pt-1.5/pb-1.5 with the plate at -top-[15px] (~6px intrusion; 46px total on a one-line quest — tighter needs a shorter plate), z-[45] so the STOPPED portcullis (its sticky range ends at the board container — that's the "stops at the quest pane" behavior) tucks its spikes behind the box's top border (-mt-[3px] tuned); board placement prompt at bottom-5 clear of the plate; HEROES gap = ENEMIES-to-lintel gap (3px h3-top-to-edge, measured). BannerMesh + .quest-banner CSS stay in-tree for wholesale revert. **`190c95e` PANEL FORGE "Quest Box" KIT (DEFAULTS_VERSION 5): capped nine-slice + QUEST plate 3-slice (plate-cap-l/mid/cap-r), new 'quest-box' family — assembler = capped sample + plate band above (2px seam; painting overlap-free), PanelNineSlice composites the plate straddling the top border ~60% proud, ground pad grows for the straddle at high zoom. Verified end-to-end with a synthetic painted sheet through the real slicer. **`5b21b4b` HONORED PAINT HALO around the detached plate (user ask): AssemblyRegion.halo insets (rep-0 only, strict axes stay exact), magenta paint box drawn in the assembled view, slicer cuts region+halo into the shipped bitmap (empty/repeat checks stay nominal), slices JSON carries insets, PanelNineSlice anchors nominal + lets bitmaps overhang; quest-box sample is now 144×152. Marker-pixel verified: halo art SURVIVES the cut — this partially closes the known "halo art is lost" forge gap, for detached pieces. Testing note: the forge's file input is a fallback — with FS Access support the real path is window.showOpenFilePicker; mock IT and click the button to drive uploads from scripts (hidden-input change dispatches silently do nothing).**** NEXT: the user paints the kit (watch loop) — then the forge's cut-pieces-into-real-game-panels renderer is the step that retires the placeholder CSS box; ALSO pending: box entrance animation (cloth's quest-drop died with it), menu-open z edge (rung z-40 behind box z-45 while gate beams z-50 above). **JSX-comment-in-expression-position parse error struck TWICE this arc — line comments only inside `return (`.** Previous: August 1, 2026 (third push) — **QUEST-TEXT + BRANDING FOLLOW-UPS (`d32a219`, `cdc9b2e`):** (a) Quest wrap fixed against the user's phone screenshot (orphaned "(2)" on its own line): every name–count pair joined by ` ` (write NBSP as the ESCAPE — literal NBSP chars trip eslint no-irregular-whitespace), article glued to its noun, quest block gets `[text-wrap:balance]`. (b) **properNoun flag on enemies + vessels** (checkbox under Plural Name in both editors; vesselToEnemyAsset + test helper carry it): grouped defeat quests now article PER PART — "Defeat Billy (1) and the Goblins (2)"; side effect: articles repeat per item ("the Skeletons (2) and the Giant Bats (3)"), grammatically correct. The user should tick it on Billy. (c) **siteSubtitleFont** branding setting: subtitle span moved `.font-medieval`→`.nav-subtitle` (unset falls through font-medieval's exact chain — no visual change until used); full font chips + preview in Settings→Branding. Verification trick used twice this session: the preview profile is LOGGED IN — Select Dungeon lists all saved puzzles, and temp localStorage flag flips (reverted immediately, storage confirmed clean) exercise asset-flag paths; NOTE assetStorage's in-memory cache means a localStorage revert needs a page reload to show. Previous: August 1, 2026 (second push) — **GAME-PAGE POLISH BATCH SHIPPED AND PUSHED (`987ec69..d388ff8`, 11 commits):** the user's nine-item list plus the facing-picker rework, one commit each, all verified live. (1) Sticky rung: exactly 3px of gate bar above the rung when scrolled (mobile ride removed — the 3px sticky offset IS the sliver; desktop rides 3px). (2) Enemy cards + drawer = the hero card design (38px name band, 14px stat line, hud registers, real buttons; measured-name-height machinery deleted; blood identity kept). (3) Quest banner: shimmer-container is now display:INLINE so long quests wrap full-width centered; (?)-gap 4px; side padding px-6/7. (4-5) DUNGEON DETAILS lintel + Items/Status Effects/Dungeon Tiles titles uppercase. (6) Those three panels' copy on the HUD ramp, identity colors kept, counters split hud-num/hud-label. (7) Hero panel -mt-[9px]: quest-text-to-HEROES gap halved 18→9px measured. (8-9) Boxed TAP A HERO / TAP AN ENEMY FOR MORE INFO chips (board-prompt grammar); the enemy one shows whenever no card is open — the one-time localStorage coach line is RETIRED. (10) **FACING PICKER (user follow-up): facing entries (isFacing flag, only on `__facing`) no longer auto-close on pick — the hub hero TURNS to the chosen bearing's idle (SpriteThumbnail gained `facingDirection`, directional-variant with default fallback); spell-aim entries keep pick-and-dismiss.** Verified end-to-end with the Knight (8-direction sprite) via the logged-in preview profile. Gates: 678 tests, tsc -b, lint 0 errors. **Watch-outs for future sessions: JSX `{/* */}` comments in EXPRESSION position (inside `&& (...)`) are a parse error — one briefly broke the dev page this session (caught, amended into `c48f8c5`); Vite kept serving a stale module graph through the error — hard-reload + check preview_logs when the page goes blank.** Player app: same components, deploys from main — this push carries everything to both sites. Previous: August 1, 2026 — **WAR TABLE ARC CLOSED: everything pushed (`516e314..d7da1fd`), `ui-mobile-fit` tagged `war-table-experiment` and deleted local+remote** (user still to flip the Netlify branch-deploy toggle off). Three changes landed first, one commit each per the pinned method, verified live on a fresh dev server (dev-alt launch config, port 5183, committed): (1) **DRAWER CEILING REVERSED (`f4d850d`)** — the user called the 190px cap + internal scroll their own error ("having to scroll inside a panel can get annoying... trying to scroll past it"); `.hero-drawer` is natural height on EVERY viewport now, wordy heroes grow the page (sanctioned), and the stated goal is hero cards + directional controls above the fold. **Do not reintroduce nested scroll regions on the game page.** `.hero-drawer__brief` and `--hero-drawer-max` are gone. (2) **Enemies/Allies register pass (`664d1a8`)** — the pending-answer question resolved YES: uppercase transform on both carved titles (identity colors kept, empty-state header included), "N remaining" split into hud-num + hud-label mirroring "N/M placed"; Allies rides (same component). (3) The three hero-panel port commits + handoff from 07-31 went out in the same push. `hero-panel-classic` retreat branch KEPT. Lint 0 errors (44 warnings), `tsc -b` clean. **IMMEDIATE NEXT: the user said "do more specific changes" after the branch cleanup — no queue was named; ask what they want to change first.** The parked art thread (painted kits replace meshes; control-rail kit-vs-mesh audit first) remains the big parked direction. Previous: July 31, 2026 (late) — **HERO-PANEL CURATED PORT COMPLETE ON MAIN.** Context: July 29–31 ran a large autonomous "War Table" mobile-fit experiment (branch `ui-mobile-fit`, Netlify branch deploy live — the user's phone reference). The user disliked its LOOK but liked its FLOW; resolution (user's own inversion): **main is the truth, good parts are ported by hand one commit at a time, and `hero-panel-classic` (= `7e69e0f`, pushed) is the wholesale retreat branch** for the hero panel. Full branch map + settled decisions live in user-memory `project_mobile_fit_state.md` — READ IT FIRST. **Landed on main today, in order:** (1) PORTCULLIS — forge plates standardized across PortcullisMesh + GateMesh (9px square at the shared 672px width, identical at every viewport: plate unit-widths sit in the meshes' 1000:400 viewBox ratio); the user chose the THIN 36px-iron menu beams over rung parity (SETTLED — comments in both meshes say so, do not relitigate); EDGE BARS added so the lattice is contained like a real portcullis (six bars/spikes/plates, outer pair flush to 0.0px, edge spikes deliberately clipped ~5px by the frame; PanelForge fracs + ref crops follow the new geometry). (2) CARD SPRITES 2× BY USER DECISION with the `?cardscale=N` (1–6) on-device override (`516e314`, PUSHED — everything after it is not): supersedes the old "never shrink the art" stance, which was judged against a rigged dpr-1 desktop comparison; still integer/native scaling. (3) UNPUSHED — the user's five-item hero-panel port list, delivered in three commits + one follow-up (`2d633cc`, `3d34143`, port 3/3, `ffafc2d`): board-overlay "Tap the dungeon to place your hero" (pointer-events-none over the bottom tiles; the inline hint line is now selection-only); HUD TYPE RAMP ported MINUS `hud-display` per the user's carve-out (`hud-title/body/label/num` on `.theme-root`, plus `--hud-gold`/`--hud-vital` roles); CARD LAYOUTS with fixed bands (38px name+epithet box, 14px stat line), gold PICK chip for unset facing-input heroes, stamped SET corner plate replacing the ✓ (art-only dim via wrapper OPACITY — never a filter over the rAF-driven canvas), cards are real `<button>`s, and movement arrows are ALWAYS animated (user explicitly overrode the experiment's selected-only rule); BOUNDED `.hero-drawer` (190px ceiling, internal scroll + fade mask, desktop unbounded) keeping the CLASSIC skin and three dashed-divider columns, with the Directions column = caption + 44px order pill (lit brass CHOOSE with a breathing dot while unset; quiet arcane outline once set) opening the ported **DirectionPicker** bottom-sheet/lg-popover (56px cells, the hero's sprite in the hub, creator-disallowed bearings as inert sockets; the 17px in-panel compass is RETIRED and the shared CompassArrow glyph now lives in DirectionPicker.tsx); HEROES title UPPERCASE (keeps carve/medieval face/themed size/purple) + the placed counter split into hud-num + hud-label registers. **THE QUEST RUNG WAS EXPLICITLY NOT PORTED — the cloth banner is untouched, per the user.** Every slice was verified end-to-end in the live browser (seeded facing-input hero: Pick chip → pill → sheet → choose → pill reads bearing → card arrow animates → sheet auto-closes) before committing. **IMMEDIATE NEXT:** (a) the user was just offered the identical uppercase/register pass for the Enemies/Allies headers + "N remaining" (now the odd ones out on the page) — ANSWER PENDING, this is the first thing to ask about; (b) the 190px drawer ceiling awaits the user's judgment against real wordy heroes (one-number tune); (c) push the four commits when the user says. **PANEL FORGE (pushed earlier on 07-31): Phase 2 complete** — in-app slicer cutting from the kit's LIVE assembly regions + a File-System-Access watch loop (paint in Aseprite, ctrl+S, the forge re-slices), live nine-slice DOM render with real sample text + width/height sliders, tile-seam overlay with clip arithmetic ("2 whole tiles + 73px clipped"), discarded-repeat reporter (a nine-slice keeps ONE period; painted later repeats are named when thrown away), divider rendering + placement slider, and the CAPPED-EDGE kit (end ornament survives resizing; sample 144×120; mid period 24 = the plain kit's so motifs transfer; caps 12 = corner). **ALWAYS bump DEFAULTS_VERSION (now 4) when editing built-in kit sizes — the localStorage copy silently wins otherwise.** Forge gaps, known: per-piece overflow HALOS are not honored by the slicer (halo art is lost — assembled workflow only); cut pieces do NOT yet render into the real game panels (that renderer is the forge's next step); the preview's divider paints under the sample text (user noted, deprioritized). **THE BIG PARKED DIRECTION** (user's vision, captured in docs/feature-backlog.md "THE PANELS SHOULD BE PART OF THE WORLD"): painted kits REPLACE the procedural meshes entirely — all of today's mesh geometry is acknowledged scaffolding. First step when unparked: audit the control-rail kit's piece list against everything PortcullisMesh actually draws; open question for the user: how rail art should tile (capped-edge style vs other). Branch endgame (tag `war-table-experiment`, delete `ui-mobile-fit`, Netlify branch deploy off) stays ON HOLD until the user says they are done with it. **Working method pinned by this arc: VISUAL WORK IS HAND-MADE** — one user-directed change per commit, judged live on the dev server against real content; the 46-agent workflow hit every metric and the user disliked it on sight (its own critics flagged the aperture-wall regression twice and it shipped anyway), while the user's itemized-list workflow landed correctly every single time. Previous: July 26, 2026 — **SETTINGS AUDIT COMPLETE AND PUSHED (`ca2d35c`, `e770245`).** The four real bugs are fixed: cross-tab Reset scoped to its own tab behind a confirm, colour defaults moved into `themeAssets.ts` and pinned against index.css by a test (which caught four drifted values), Inter made selectable for the heading/menu fonts, and failed imports/saves now reach the error banner — import used to accept ANY valid JSON and write it straight over the theme. Finding 9 answered: the entity/asset preview colours ARE live (read via `getPropertyValue`, invisible to grep). **Then the bigger find: 44 of ~140 theme settings had no consumer anywhere — the entire Defeat Panel tab, everything on Action Buttons except the three images, seven icons, and three compendium keys whose category was not even in `ASSET_CATEGORIES`. All deleted with the user's approval; the Effects tab went with them (its only two settings were dead).** That was the real answer to "the Settings page is a mess" — and it closes the last open item in the 2026-07-21 working batch, so **there is no agreed next feature; ask.** Also corrected this session: the "feature queue: projectile linger, hero behavior slots" line that four session headers (including this one, at first) carried forward — **both shipped back in July**, linger on 07-16 and behavior-slots-as-aimed-spells on 07-17, and the queue was declared empty on 07-17. The user caught it. Previous: July 25, 2026 — **DEV-PAGE RESTYLE PASS COMPLETE AND PUSHED** (21 commits, `6b762ee`..`4339ace`, including the Settings page). **NEXT SESSION STARTS WITH THE SETTINGS AUDIT — ten findings are gathered and waiting on user decisions, see "Settings audit — findings" under "Next session — start here".** Also new and important: a **"Testing Philosophy" section near the top** — the AWAITING USER TEST markers throughout this doc are NOT a backlog to clear, per the user directly. All five Puzzle Resources tabs and all twelve Asset Manager tabs now follow the Production standard, and every asset editor gained **browse-vs-edit full-width modes** (a design round with the user, not just a restyle — see "Asset Editor Browse Mode" and "Next session — start here"). Also fixed en route: signed-in creators being bounced off gated routes by an AuthContext race; showcase heroes not counting as usages (deleting one silently broke Slab demos); unguarded `pickupPermissions` throwing in the editor and mid-turn; colliding asset ids; missing folder categories for vessels + sounds. **AWAITING USER TEST on multiple devices.** Previous: July 21, 2026, second session — FIVE arcs closed: (1) SHOWCASE DISTRIBUTION + SLAB REVEAL TIMING (publish stamps, shared reveal predicate, and player asset CLOSURE PREFETCH — a brief boot pull-all was superseded same-session by `be21e08` after a second design round with the user; top entry under Pending Tasks); (2) CONTENT PRODUCTION DASHBOARD (Puzzle Resources "Production" tab + auto-refresh; now the user-set styling reference for all dev pages — see "Dev-Page Styling Reference" section); (3) HOMING-REFLECT TIMING DIVERGENCE closed — known real/headless divergence ledger EMPTY; (4) CI's vacuous Type check fixed (`tsc -b` required — plain `tsc --noEmit` checks ZERO files here); (5) re-publish now refreshes ALL deps (edited live assets finally reach assets_live). Everything AWAITING USER TEST. Session tail: player asset pull-all SUPERSEDED by closure prefetch (`be21e08`) + LoadingRune plumbing (`32ac1cb`, art pending). **NEXT SESSION (user-directed): dev-page restyle pass to the Production standard — see "Next session — start here".** Previous: July 14, 2026, third session (MAP EDITOR REDESIGN COMPLETE — Phase 1 decomposition, Phase 2 layout rework, Phase 3 interaction gestures + mobile, all user-approved along the way, dungeon-theming pass CANCELLED by the user; plus 2 new theme fonts. See "Recently completed (July 14, third session)". Earlier the same day: HIT-STAMP CONDITIONS closed out the trigger overhaul — that whole batch still AWAITS USER TESTING on deploy. NOTE: the July 1–12 work — engine audit sweeps 1–10, summon/necromancy/vessels, Phase E homing helpers, strafe actions, contact redesign — is chronicled in the user-memory `in-progress.md`, not here; this doc's session log resumes at June 30 below.)
...
| **Player app vision/architecture** | `docs/PLAYER_APP_VISION.md`, `docs/PLAYER_APP_ARCHITECTURE.md` | Player site separation reference. ⚠️ ARCHITECTURE's on-demand registry was NEVER built; 2026-07-21 shipped **CLOSURE PREFETCH** instead (`be21e08`, second design round — a brief boot pull-all was superseded the same day): utils/livePull.ts `ensureAssetsLocal`/`ensurePuzzleAssets`, one async choke point per surface (daily boot / Slab+Training via usePlayerReveal / demo tap / arena select), revealed-only + per-id day-freshness ledger whose no-ledger-entry rule protects editor working copies. |
...
built):** shipped as boot-time PULL-ALL, then **SUPERSEDED the same
session by CLOSURE PREFETCH (`be21e08`)** after a second design round —
the user wanted no ceiling race, fast boot, and NOTHING undebuted on a
player device, and correctly noted the Slab only renders some assets at
a time. Current shape: utils/livePull.ts `ensureAssetsLocal(ids)`
(batched fetch + per-id day-freshness ledger; a locally-present id with
NO ledger entry is NEVER fetched — protects editor working copies) and
`ensurePuzzleAssets(puzzle)` (stamp, or client fixpoint walk for
unstamped rows). One async choke point per surface: daily boot ensures
its closure before the swap (Game.tsx), usePlayerReveal ensures the
revealed set (Slab lists + Training roster), ShowcaseSection fetches
demo JSON per REVEALED page + ensures on tap ("Summoning…" beat),
arena select ensures. SNOOP-HOLE also fixed: the live-content cache
holds a showcase id→entityIds INDEX, never full demo JSON (v2 cache
key); full rows come via fetchLivePuzzlesByIds only for revealed
pages. Storage grows with what a player touches, not the library.
Pins in live-pull.test.ts (ledger safety, fixpoint, stamped path). **`9c67ce6` surfaces:** utils/reveal.ts =
...
**⚠️ THE "projectile linger, hero behavior slots" QUEUE LINE IS STALE —
BOTH SHIPPED IN JULY, DO NOT PICK THEM UP.** Projectile linger shipped
2026-07-16 (`239804f` engine + `2eb6d48` visual, `lingerDuration`, 7
pins in projectile-linger.test.ts). "Hero behavior slots" was clarified
by the user on 2026-07-17 to mean the redirect compass generalized, and
shipped the same day as player-aimed spell direction (`ce989e0` +
`ee3f342`, `SpellAsset.directionAcceptsUserInput`). The queue was
declared EMPTY on 07-17 (see the vessel-triggers entry) and the line
survived anyway, echoed forward through four session headers including
this one. **When a queue line and a dated ship note disagree, the ship
note wins — check `docs/feature-backlog.md` before starting anything.**

**There is no agreed next feature.** The 2026-07-21 working batch is
fully shipped, and its last open item — the Settings tab cleanup — was
closed 07-26. What remains in the backlog is unrequested: the two
leftover user-input variants (**pick a placed hero's initial facing**,
**pick which spell from a slot** — genuinely unbuilt, no flags exist),
eyes-in-the-dark, the passerby, the developer badge, and the post-launch
roguelike mode. **Ask the user what they want next.**
...
- ~~Feature queue after the audit: projectile linger, hero behavior
  slots~~ — **STALE, both shipped in July.** See "Next session — start
  here".

### Settings audit — what is fixed and what is waiting (2026-07-26)

**Findings 1–7 are FIXED and pushed (`ca2d35c`).** What that commit did,
and the traps inside it:

- **Reset is per-tab now.** `handleResetCategory` replaces the two
  cross-tab handlers; it clears only the active category, only keys
  that are actually set, behind a `window.confirm` naming the count.
  It is offered only where every setting on the tab is a colour or a
  select — image uploads stay one card at a time, because clearing the
  key would orphan the Supabase Storage object.
- **`THEME_COLOR_DEFAULTS` now lives in `utils/themeAssets.ts`**, not
  the editor, transcribed from the CSS fallbacks and pinned against
  index.css by `src/utils/__tests__/theme-settings.test.ts`. Writing
  that test caught four drifted values in the old map (success
  `#556b2f` vs the real `#3d4d21`, danger `#c12525` vs `#841919`,
  navbar `#080706` vs `#3a332b`, sprite preview `#1f1810` vs
  `#15100a`). **A key is deliberately ABSENT when its unset state is
  not a flat colour** (a Tailwind gradient, a `dungeon-btn` plate) —
  those cards render a hatched swatch and "not set" rather than invent
  one. `isCustom` is now just "a value is stored".
- **Fonts:** `'default'` now MAPS to the Inter stack instead of being
  skipped. It has to — `--theme-font-family-heading` and
  `--theme-font-family-menu` fall back to Almendra, so clearing the key
  was the one thing that could never produce Inter. `StyleSelector`
  gained an explicit **Default chip** (clears the key) and every other
  chip now stores its literal value; that also retired the guessed
  `defaultValue`, whose "which chip is lit when unset" indicator
  disagreed with the CSS on border radius and width (finding 6).
  **Font SIZE still deliberately skips `'medium'`** — `.font-medieval`
  falls back to `inherit`, which a written-out 16px would override.
- **Failures reach the banner.** `importThemeAssets` used to accept any
  valid JSON and write it straight over the theme (a number or an array
  wiped every setting) and returned true regardless; it now validates
  shape, requires at least one recognisable key, and returns
  `ThemeSaveResult`. `setThemeAsset`/`deleteThemeAsset` return their
  save result too, so a full localStorage no longer discards an edit in
  silence (finding 7).

**Finding 9 is ANSWERED: `colorBgPreviewEntity` and
`colorBgPreviewAsset` are live.** They are read through
`getPreviewBgColor(type)` via `getPropertyValue`, not from index.css —
which is why a plain grep says "unused". They reach every
`SpriteThumbnail previewType="entity"|"asset"` (game panels, compendium,
asset editors) and `StaticSpriteEditor`'s canvas. The genuine confusion
is that **the `.sprite-preview-bg` CSS class reads ONLY
`--theme-bg-preview`** — set just the entity/asset colours and every
...
- **Defeat Panel — the entire tab (6 keys).** No component reads any
  ...
  `gameOverPanel*` + the `defeat-panel` class.
- **Action Buttons — everything except the three `*Image` keys.** All
  12 colour keys (Bg/Border/Text × Play/TestHeroes/TestEnemies/Concede),
  all 4 `*Shape` selects, all 3 `*ImageDisabled` keys are inert. Only
  `actionButtonPlayImage`, `actionButtonTestHeroesImage` and
  `actionButtonTestEnemiesImage` are consumed.
- **`colorAccentMagic`** sets `--theme-accent-magic`, which nothing reads.
- **`shadowIntensity`** — the whole Styles-tab shadow control is inert.
- **`bgGameArea`**, **`borderFrameSmall`**, **`buttonSecondary`**,
  **`buttonSuccess`**, **`buttonDanger`**, **`overlayNoise`**,
  **`overlayVignette`**.
- **Icons: `iconBugReport`, `iconShield`, `iconSword`, `iconNavPlay`,
  `iconNavEditor`, `iconNavAssets`, `iconNavCompendium`.**
- **The three `compendium*` texture keys** are doubly dead: they carry
...
`borders` share the 🖼️ icon). **11** (`FxSettingsPanel` mixes the frame
profiler HUD and freeze-board-drawing diagnostics in with real settings)
is a grouping decision, also unacted.

<details><summary>Original findings list as gathered 2026-07-25 (line numbers as of <code>24b59a9</code>)</summary>
...
feedback rounds to interleave. (The old "Settings audit ON HOLD" note
here was from 2026-07-21 and is superseded: the audit ran on 07-25 and
its bugs were fixed on 07-26. The "feature queue: projectile linger,
hero behavior slots" note that also sat here was stale — both shipped in
July; see "Next session — start here".)
...
**THE REDESIGN IS DONE → the feature queue unparks: projectile linger, hero behavior slots, vessel triggers** (homing-reflect timing stays pinned, plan doc §Phase E). — ⚠️ **HISTORICAL. This sentence is where the stale queue line came from: all three shipped 07-16/07-17, but it was copied into four later session headers as if still pending. Do not carry it forward again.** **Also pending:** user testing of the whole 2026-07-14 batch on the deployed site — trigger overhaul + hit stamps + instant dispel (damage-smoke pass) AND the full editor redesign (desktop: toolbar/tabs/roster/drag/right-click/inspect; phone: finger drag, long-press delete incl. on a stacked placement, no page-scroll while painting).
